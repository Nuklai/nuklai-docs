...
`_txID_` _is the_ `_assetID_` _of your new asset._

The "loaded address" here is the address of the default private key (`demo.pk`). We use this key to authenticate all interactions with the `nuklaivm`.
...
Unlike the mint demo, the AWM demo only requires running a single command. You can kick off a transfer between the 2 Subnets you created by running the following command from this location:
...
_The_ `_export_` _command will automatically run the_ `_import_` _command on the destination. If you wish to import the AWM message using a separate account, you can run the_ `_import_` _command after changing your key._